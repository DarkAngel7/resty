...
func TestParseRequestBody(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedContentLength >= 0 {
				if int64(tt.expectedContentLength) != r.ContentLength {
					t.Errorf("Content-Length header = %v does not match expected %v", r.ContentLength, tt.expectedContentLength)
				}
			}
